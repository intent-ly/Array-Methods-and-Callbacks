...
let finalGameInfo = fifaData.filter(function(item){
    return item.Year == 2014 && item.Stage == "Final";
});

console.log(finalGameInfo[0]["Home Team Name"]);
console.log(finalGameInfo[0]["Away Team Name"]);
console.log(finalGameInfo[0]["Home Team Goals"]);
console.log(finalGameInfo[0]["Away Team Goals"]);
console.log(finalGameInfo[0]["Home Team Name"]);
...
accepts the callback function `getFinals`, and returns an array called `years` containing all of
the years in the dataset */

function getYears(finals){
    const yearArray = finals.map((item)=>{
     return item.Year;
   })
   return yearArray;
 }
...
function getWinnersByYear(winners, years) {
    ...
    for (let i =0; i < winners.length; i ++){
        container.push (`In ${years[i]}, ${winners[i]} won the world cup !`);
    }
    return container;
};
...
function getAverageGoals(data) {
    let homeAverage= data.reduce((acc, score)=>{
        return acc + score["Home Team Goals"];
    }, 0)
    let awayAverage= data.reduce((acc, score)=>{
        return acc + score["Away Team Goals"];
    }, 0)

    return{"Home Average": homeAverage/ data.length, "Away Average": awayAverage/ data.length}
